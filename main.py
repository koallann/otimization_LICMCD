...
def alocar_drones(Jopen, C, bij, B, nd):
  ...
  while total_atribuido < nd:
    print(f"{total_atribuido} < {nd}")
    
    for j, limite in Jorder:
      print(f'j: {j} / limite: {limite}')
...
np.random.seed(42)
wi = np.random.uniform(1, 5, size=n)
dij = np.random.uniform(100, 1000, size=(n, m))  # em metros
